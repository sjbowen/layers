...
var rhizomeUsed = false; // set to true if rhizome is being used as web server
var webPdUsed = true; // set to true if adding audio via webPd

var readyElement = document.querySelector('#ready');

if (webPdUsed && readyElement!=null) {
	readyElement.addEventListener('click', function () {
		if (debugMode) {console.log('click');}
		readyElement.innerHTML = ''; // hide the ready link
		var patch
			$.get('patches/myPatch2.pd', function(patchStr) {
	...
	readyElement.addEventListener('touchend', function () {
		if (debugMode) {console.log('touch');}
		readyElement.innerHTML = ''; // hide the ready link
		var patch
			$.get('patches/myPatch2.pd', function(patchStr) {
			  patch = Pd.loadPatch(patchStr)
	...
	if (readyElement!=null) {readyElement.innerHTML = ''}; // hide the ready link
	go();
}
...
// 	  controls.registerMethod('outElement',   new Marzipano.ElementPressControlMethod(viewOutElement, 'zoom',  velocity, friction), true);
	  ...
	  // Create layers instead of scenes and add them to the stage
	  var layers = APP_DATA.scenes.map(function(sceneData) {
		...
		layer.setEffects({opacity: 0.5}); // set an initial opacity of 0.5
		console.log(layer);

		// Create link hotspots.
		sceneData.linkHotspots.forEach(function(hotspot) {
		  var element = createLinkHotspotElement(hotspot);
		  layer.hotspotContainer().createHotspot(element, { yaw: hotspot.yaw, pitch: hotspot.pitch });
		});
		...
		sceneData.embedHotspots.forEach(function(hotspot) {
			...
			layer.hotspotContainer().createHotspot(element, { yaw: hotspot.yaw, pitch: hotspot.pitch }, { perspective: { radius: hotspot.radius, extraRotations: hotspot.extraRotations }});
		});		
		
		// Add Gaze Spots by adding event listener for viewChange on the whole layer  Gazespots should be for all layers?
		layer.addEventListener('viewChange', function() {
			var yaw = layer.view().yaw();
			var pitch = layer.view().pitch();
			var gazing = false;
			...
			//for each gazespot, check if view closely matches, set gazing to true if so
					...
					if (timer == null) { // if a timer hasn't already been started, start one
							if (debugMode) {console.log("gazeSpot" + gazeSpot.target + " found, gaze timer started")};
							...
							if (webPdUsed) {Pd.send('send6', [parseFloat(gazeSpot.target.charAt(0))])}; // tell webPd which gaze spot we've found (scene id number)
						}
					gazing = true;
					}
				});
			if (!gazing) { // if not gazing 
					if (timer != null) { // and if the timer has not already been cleared
						...
						timer = null;
						if (debugMode) {console.log("off gazespot");}; 
		...
		return {
		  data: sceneData,
		  marzipanoObject: layer
		};
	  });

// 	Display all the layers?
// 	console.log(layers);
// 	layers.forEach(function(layer) {stage.addLayer(layer)});
...
//	  var scene = viewer.scene(); // get the current scene
	  var view = viewer.view();    // get the scene's view
...
// 	  });
	  ...
	  // Simon's handler for view change, use to send GYRO position data to webPd or rhizome 
	  ...
	  // get yaw, pitch, vertical fov from current view (in radians)
		...
		if (rhizomeUsed) {
			...
			var rhizomeYaw = (yaw+Math.PI)/Math.PI*2;
			var rhizomePitch = (pitch+(Math.PI/2))/Math.PI;
			var rhizomefov = fov/Math.PI;
			throttledSend ([rhizomeYaw, rhizomePitch, rhizomefov]); // send data to rhizome
		};
		if (debugMode) { debugElement.innerHTML = 'yaw' + wpdYaw + '<br />pitch' + wpdPitch + '<br />fov' + wpdfov; };
		});	
...
// function definitions
	  ...
	  function switchScene(scene) {
		...
		if (webPdUsed) { // send new scene id to webPd
      	...
      	if (rhizomeUsed) {rhizome.send ('/scene', [parseFloat(scene.data.id.charAt(0), 100) / 4])}; // tell rhizome that we've changed scene
